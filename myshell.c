...
FILE *fp1,*fp2,*fp;
void Create(); void Append();
void Delete(); void Display(); void TextEditor();
void cd();
void listHistory();
...
char input[50] = ""; char file_name[30];
char process_name[30];
while(strcmp("exit",input)!=0){ printf("\n>>"); scanf("%s",input);
newCmd(input);
...
be performed for 2 integers with no spacing.\n");
...
printf("10) run - Runs any application!\n");
printf("11) exit - Exits the shell!");
}
...
else if(strcmp("gcc",input) == 0){ char system_call[100]; scanf("%s",file_name);
...
else if(strcmp("start",input) == 0){ char system_call[100]; scanf("%s",file_name);
sprintf(system_call, "%s %s", input, file_name); int error = system(system_call);
if(error == 0){
printf("Compilation has been successfull!\n");
}
}
else if(strcmp("run",input) == 0){
char system_call[100]; scanf("%s",process_name);
...
else if(isdigit(input[0])){ float first, second;
if(strchr(input,'+')){
sscanf(input,"%f+%f",&first,&second);
printf("%f",first+second);
}
else if(strchr(input,'-')){ sscanf(input,"%f-%f",&first,&second); printf("%f",first-second);
}
else if(strchr(input,'*')){ sscanf(input,"%f*%f",&first,&second); printf("%f",first*second);
}
else if(strchr(input,'/')){ sscanf(input,"%f/%f",&first,&second); printf("%f",first/second);
}
else if(strchr(input,'^')){ sscanf(input,"%f^%f",&first,&second);
...
else if(strcmp("editor",input) == 0){ printf("\nWelcome to the basic text editor!");
TextEditor();
}
...
{
fclose(fp1);
printf("\nEnter then new filename: "); scanf("%s",fn); fp1=fopen("temp.txt","r");
fp2=fopen(fn,"w");
while(!feof(fp1))
{
c=getc(fp1);
...
void Display()
{
printf("\nEnter the file name: "); scanf("%s",fn); fp1=fopen(fn,"r"); if(fp1==NULL)
{
printf("\n\tFile not found!"); goto end1;
}
while(!feof(fp1))
{
c=getc(fp1);
...
fclose(fp1);
if(remove(fn)==0) {
printf("\n\nFile has been deleted successfully!"); goto end2;
}
else printf("\nError!\n");
end2:
printf("\n\nPress any key to continue!");
...
{
char *buf;
buf=(char *)malloc(100*sizeof(char)); getcwd(buf,100);
...
void newCmd(char* cmd) { strcpy(history_list[last].history,cmd);
last++;
}
void listHistory(){
